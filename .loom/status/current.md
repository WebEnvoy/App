...
- Latest Validation Summary: 2026-07-11T16:37Z: `npm run typecheck`, `npm run smoke`, `npm run smoke:packaged:readonly`, and `git diff --check` passed. App sends Core-main-compatible query-only `public_query`, keeps city/page/limit internal, renders keyword/city/limit controls, and mock POST smoke proves `/tasks` accepts the BOSS payload. No live account/site action occurred.
