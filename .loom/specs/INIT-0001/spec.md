...
- Restore the App repository Loom bootstrap fact chain so the Stage 5 Library work can bind to a real repository carrier instead of an init placeholder.
- Keep this change limited to governance carrier metadata; no product behavior, runtime code, credentials, package truth, session truth, or evidence payloads change here.

## Suite Path

- Suite path: minimal
- full-path-artifacts not_applicable rationale: bootstrap repair only exercises the minimal formal suite needed to consume INIT-0001 carrier health; consumer boundary: suite validation, spec review, review, merge-ready, and controlled merge must not require suite-index.md, research.md, contracts.md, or readiness-checklist.md for this bootstrap-only repair; recheck condition: switch to full suite when the bootstrap PR carries product behavior, package truth, runtime truth, external writes, or shared contract changes.
...
- Out of scope: Stage 5 Library UI behavior, Lode catalog truth, Core task intent truth, Harbor runtime evidence truth, and any Stage 6 behavior.

## Behavior Evidence

- Story scenario mapping: No user-facing story scenario is changed by this bootstrap carrier repair.
- Story business confirmation locator: Stage 5 story readiness is carried by the downstream App Library Story Readiness PR, not INIT-0001.
...
- Operational boundaries: No live account, profile, browser runtime, credential, cookie, raw DOM, raw network, or profile storage is introduced.
