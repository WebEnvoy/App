...
| --- | --- | --- | --- | --- | --- | --- | --- |
| EV-001 | behavior_evidence | src/renderer/coreTaskSubmitClient.ts | APP-240 acceptance criteria | Structured BOSS one-shot submit and authenticated official Chrome restricted-fallback admission | present | App admission only; warnings remain Harbor-owned facts and no live task is executed by this PR | Refresh after submit contract or admission changes. |
| EV-002 | test_evidence | scripts/smoke.mjs | EV-001 | BOSS restricted Chrome plus proxy warning positive case; needs-auth, fixture/local/offline, write/precheck, bulk, and malformed query negative cases | present | Local contract smoke; no external BOSS action | Rerun after App admission, identity projection, or Core submit behavior changes. |
