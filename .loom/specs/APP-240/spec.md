...
- [ ] An authenticated Harbor live identity using official Chrome restricted fallback is admitted for one read-only BOSS job-search only when `provider_conflict` and `fingerprint_conflict` are present; `proxy_missing` is tolerated only as an additional BOSS warning. Xiaohongshu or arbitrary read specs cannot consume that exception, and needs-auth, fixture/local/offline, write/precheck, bulk, proxy-only, or unknown-warning inputs remain blocked.
