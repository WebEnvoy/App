...
它承载任务、能力资产、运行记录、证据、异常处理，以及 Harbor 提供的 Profile、Runtime Session、Viewer 和人工接管界面。

WebEnvoy App 不是 Core Runtime，也不是 Harbor Runtime，也不是 Lode 资产仓库。所有任务执行仍通过 WebEnvoy API Server 和 Core Runtime；浏览器身份和运行现场仍通过 Harbor Runtime API；网站经验、能力包和任务封装仍来自 Lode。
...
App 可以调用 WebEnvoy API Server 和 Harbor Runtime API，但不应绕过 Core 写任务记录，不应保存 Lode 能力资产真相，也不应绕过 Harbor API 操作浏览器会话。
...
- Library：站点、能力、任务模板、测试样例、版本和失效状态；
