...
它面向市场、运营、产品经理等非技术用户，承载确定性网页 workflow 的自动执行入口、WebEnvoy 全局运行事实观测、站点技能管理、账号身份管理、浏览器执行现场和异常处理入口。

WebEnvoy App 不是 Core Runtime，不是 Harbor Runtime，也不是 Lode 资产仓库。自动任务执行通过 WebEnvoy API Server 和 Core Runtime；站点技能、能力包和任务封装来自 Lode；账号身份、浏览器环境和执行现场通过 Harbor Runtime API。
...
- 选择站点技能、账号身份和业务输入，运行 Lode 封装的确定性 workflow；
...
这个定义约束 App 的自动 workflow 任务。Agent、API、CLI、MCP、SDK、skills 或其他上层应用不受 App 自动执行入口限制；它们产生的运行事实仍应在 App 中可观测。
...
没有合适站点技能时，用户可以从 Browser 或账号身份入口启动受控浏览器实例；这不是自动任务执行，也不能被标记为 task success。
...
- Harbor 负责账号身份、浏览器环境、Runtime Session、Viewer、人工接管、provider 能力事实和运行证据。
