...
目标用户是市场、运营、产品经理等非技术用户。他们需要选择站点技能、选择账号身份、填写业务输入、执行确定性任务、查看结果依据和执行现场，也需要观测 Agent/API/CLI/MCP 等非 App 调用方产生的运行事实。
...
- Library 和 Browser 管理台仍存在：Library 管理站点技能和能力资产，Browser 管理账号身份、浏览器环境和 Runtime Session。
...
没有合适站点技能时，自动任务入口不可用；用户仍可从账号身份或 Browser 管理面启动受控浏览器实例，进行手动浏览、登录、观察或准备环境。
