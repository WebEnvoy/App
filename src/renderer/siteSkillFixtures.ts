...
export const siteSkillFixtures: SiteSkill[] = [
  {
    ...
    packageRef: "lode://site-capability/xiaohongshu/search-notes@0.1.0 + lode://site-capability/xiaohongshu/read-note-detail@0.1.0",
    lockRef: "lode://lock/site-capability/xiaohongshu/search-notes@0.1.0",
    fetchedAt: "2026-07-06T09:20:00Z",
    sourceHealth: { label: "demo only", status: "fixture", detail: "缺少 live Core/Harbor/runtime evidence 时只能展示 demo projection。" },
    tags: ["小红书", "搜索", "笔记读取", "只读"],
    inputTemplates: ["搜索词", "笔记 URL/ID", "小红书身份环境"],
    readiness: [
      { label: "Lode metadata", status: "fixture", detail: "小红书 search-notes/read-note-detail capability refs fixture 已锁定" },
      { label: "Harbor identity", status: "fixture", detail: "小红书运营号 A fixture，不作为 live identity" },
      { label: "Core run", status: "unavailable", detail: "需要 live Core /capability-runs、/runs/:id/result 和 evidence refs" },
    ],
    boundaries: [
    ...
    recentTest: {
      label: "XHS read-only Core owner query",
      status: "passed",
  ...
  },
  {
    ...
    packageRef: "lode://site-capability/boss/job-search@0.1.0 + lode://site-capability/boss/read-job-detail@0.1.0",
    ...
    readiness: [
      { label: "Lode metadata", status: "fixture", detail: "BOSS job-search/read-job-detail capability refs fixture 已锁定" },
    ...
    recentTest: {
      label: "BOSS read-only Core owner query",
      status: "blocked",
  ...
  {
    id: "xiaohongshu-write-preview",
    name: "小红书发布草稿写前预览",
    category: "内容发布",
    description: "从小红书发布器真实页面生成草稿写前验证，展示预期变化、证据、风险和未发布状态。",
    status: "fixture",
    source: "Lode fixture",
    ...
    packageRef: "lode://package/xiaohongshu-write-pre-preview@0.1.0",
    lockRef: "lode://lock/xiaohongshu-write-pre-preview/2026-07-06",
    fetchedAt: "2026-07-06T10:18:00Z",
    sourceHealth: { label: "write-pre fixture", status: "fixture", detail: "Core、Harbor、Lode fixture 只能投影 demo 写前验证；不能作为真实可运行。" },
    tags: ["小红书", "写前验证", "草稿", "未发布"],
    inputTemplates: ["草稿标题", "正文素材", "话题", "小红书身份环境"],
    readiness: [
      { label: "Lode expected_change", status: "fixture", detail: "发布草稿写前能力 metadata fixture available" },
      { label: "Harbor composer refs", status: "fixture", detail: "小红书发布器字段 ref 和截图 ref 仅为 fixture" },
      { label: "Core action request", status: "fixture", detail: "submitted=false / 未提交" },
    ],
    boundaries: [
      "只展示写前验证；不点击发布，不进入 Stage 7。",
      "App 不保存 raw evidence、Cookie、token、profile storage、DOM、HAR、trace、video 或 network body。",
    ],
    relatedTaskIds: ["task-xhs-publish-write-preview"],
    recentTest: {
      label: "XHS write-pre fixture test",
      status: "passed",
      ranAt: "2026-07-06T10:18:00Z",
      postCheck: "submitted_false_visible",
      failureReason: "none",
      source: "Core fixture",
    },
  },
  {
    id: "boss-greeting-write-preview",
    name: "BOSS 打招呼写前预览",
    category: "招聘",
    description: "从 BOSS 候选人消息框生成打招呼写前验证，展示目标职位、文案、风险和未发送状态。",
    status: "needs-identity",
    source: "Lode fixture",
    ...
    packageRef: "lode://package/boss-greeting-write-pre-preview@0.1.0",
    lockRef: "lode://lock/boss-greeting-write-pre-preview/2026-07-06",
    fetchedAt: "2026-07-06T10:22:00Z",
    sourceHealth: { label: "identity needs auth", status: "needs-identity", detail: "BOSS 招聘号可能需要人工认证，写前验证仍保持 no-submit。" },
    tags: ["BOSS", "打招呼", "写前验证", "未发送"],
    inputTemplates: ["目标职位", "候选人 ref", "消息模板", "BOSS 身份环境"],
    readiness: [
      { label: "Lode expected_change", status: "fixture", detail: "打招呼写前能力 metadata fixture available" },
      { label: "Harbor message refs", status: "needs-identity", detail: "招聘号可能需要扫码或二次验证" },
      { label: "Core action request", status: "fixture", detail: "submitted=false / 未提交" },
    ],
    boundaries: [
      "只展示写前验证；不点击发送、不投递、不进入 Stage 7。",
      "App 不保存聊天材料、简历、raw evidence、Cookie、token 或 profile storage。",
    ],
    relatedTaskIds: ["task-boss-greeting-write-preview"],
    recentTest: {
      label: "BOSS greeting write-pre fixture test",
      status: "blocked",
      ranAt: "2026-07-06T10:22:00Z",
      postCheck: "login_recovery_required",
      failureReason: "needs_identity",
      source: "Core fixture",
    },
  },
  {
    ...
    sourceHealth: { label: "demo only", status: "fixture", detail: "Core、Harbor、Lode fixture 只能用于 demo projection。" },
    tags: ["淘宝", "商品页", "只读"],
    inputTemplates: ["商品详情 URL", "账号身份", "字段清单"],
    readiness: [
      { label: "Lode metadata", status: "fixture", detail: "capability package fixture available" },
      { label: "Harbor identity", status: "fixture", detail: "运营账号 A fixture，不作为 live identity" },
  ...
  },
  {
    ...
    readiness: [
      ...
      { label: "Write boundary", status: "fixture", detail: "App fixture forbids live write" },
    ],
  ...
  },
  {
    ...
    readiness: [
      { label: "Local config", status: "fixture", detail: "endpoint choice stored locally" },
